...
const appId = "YOUR_APP_ID"; // Replace with your actual Agora App ID
...
client.on("user-published", async (user, mediaType) => {
    ...
    // This event is triggered when a remote user publishes a video or audio stream.
    // You can subscribe to the user's stream at this point.
});

// Join a channel
async function joinChannel() {
    try {
        // Replace 'YOUR_TOKEN' and 'YOUR_CHANNEL_NAME' with actual values
        const token = "YOUR_TOKEN";
        const channelName = "YOUR_CHANNEL_NAME";

        await client.join(appId, channelName, token, null);

        // Start local audio and video streams if needed
        // ...

    } catch (error) {
        console.error("Joining channel failed:", error);
    }
}

// Call the joinChannel function to join a channel
joinChannel();
//#2
let config = {
    appid:null,
